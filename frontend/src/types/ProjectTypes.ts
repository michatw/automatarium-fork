 ...
 * - T: Task
 */
export type ProjectType = 'FSA' | 'PDA' | 'TM' | 'T'
...
/**
 * Alias to make Task seem like the rest
 */
export type TaskAutomataTransition = BaseAutomataTransition
...
/**
 * A gradable project Graph consists of a task and a seperate solution. To ease grading all states are stored within the same
 * array. Which state should be shown in the UI is destinguished by a flag called 'mode'. Other than the base project Graph
 * the gradable project Graph has two initials states.
 */
type GradeableProjectGraph<PT extends ProjectType, T extends BaseAutomataTransition> = {
    projectType: PT
    states: AutomataState[]
    transitions: T[]
    initialState: number | null
...
/**
 * Graph for Task project. There isn't any different in types but this is just for completion’s sake
 */
export type TaskProjectGraph = GradeableProjectGraph<'T', TaskAutomataTransition>
 ...
 */
export type ProjectGraph = FSAProjectGraph | PDAProjectGraph | TMProjectGraph | TaskProjectGraph
...
export type Template = CopyDataProject & {
    _id: string,
    name: string,
    date: number
}
