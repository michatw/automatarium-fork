  ...
  const saveComment = () => {
    if (value && !/^\s*$/.test(value) && dialog.type === 'comment') {
      if (dialog.selectedComment === undefined) {
        ...
        useProjectStore.getState().createComment({ x: pos[0], y: pos[1], text: value.trim(), mode: mode})
      } else {
        ...
        <Dropdown {...DROPDOWN_PROPS}>
          <InputWrapper>
            <Input
              ...
              onKeyUp={handleSave}
              placeholder={'λ'}
              maxLength={1} // Max length is set to 1 to prevent multiple characters
