...
import { CardContainer, CardDetail, CardImage, SelectedTemplateOverlay, TitleAndKebab, TypeBadge } from '../ProjectCard/projectCardStyle'
import { ExpressionType } from '/src/types/ExpressionTypes'
dayjs.extend(relativeTime)

type ExpressionCardProps = {
  name: string
  type?: ExpressionType | '???' // '???' is used has a default type
  date: string | Dayjs
  image?: string
...
const ExpressionCard = ({ name, type, date, image, isSelectedTemplate = false, ...props }: ExpressionCardProps) => {
  const { ...rest } = props
