...
import { ghostStyles, pathSelectedClass, pathStyles } from './transitionSetExpressionStyle'
...
/*
  * !!Warning!!
  * This file still needs to be refactored properly. At the moment it's kind of a working hacky solution as it still mixes Projects and Expressions.
*/
...
const TransitionSetExpression  = ({ transitions, isGhost = false, isTemplate = false } : {transitions: PositionedTransition[], isGhost?: boolean, isTemplate?: boolean}) => {
  const projectType = 'FSA'
  ...
  const renderTransition = (toRender: PositionedTransition[], bend: BendDirection) => {
    ...
    if (isGhost) {
      return <TransitionSetExpression.Ghost {...props}/>
    } else {
      return <Transition
...
TransitionSetExpression.Transition = Transition

TransitionSetExpression.Ghost = ({ from, to, bendDirection }) => {
  const pathID = `${from.x}${from.y}${to.x}${to.y}`
  const bendValue = getBendValue(bendDirection)
...
export default TransitionSetExpression
