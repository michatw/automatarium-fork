...
import { MousePointer2, Hand, ChevronDown, Circle, ArrowUpRight, MessageSquare } from 'lucide-react'
...
import { ToolPopup, ToolName, ToolHotkey, Animation } from './toolbarExpressionStyle'
...
const ToolbarExpression = ({ mode, isTask }) => {
        ...
        <SidebarButton
          ...
          onMouseEnter={e => {
            ...
            window.setTimeout(() => {
              if (toolPopupHover.current.value !== toolOption.value) return
              const box = (e.target as HTMLElement).getBoundingClientRect()
              setToolPopup({ visible: true, y: box.y, tool: toolsTask.find(t => t.value === toolOption.value) })
            }, toolPopupHover.current.timeout || 1000)
          }}
          onMouseLeave={e => {
            toolPopupHover.current = { value: undefined, timeout: ((e.relatedTarget as HTMLElement).tagName === 'BUTTON' && toolPopup.visible) && 10 } as ToolHoverType
