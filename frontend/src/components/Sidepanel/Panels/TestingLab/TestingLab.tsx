...
import { useProjectStore, usePDAVisualiserStore, useMetaDataStore } from '/src/stores'
import { closureWithPredicate, simulateFSA, simulatePDA, simulateTask } from '@automatarium/simulation'
...
const TestingLab = () => {
  ...
  const showTask = mode === 'TASK'
  const showSolution = mode === 'SOLUTION'
  ...
  const statePrefix = showTask? useProjectStore(s => s.project.config?.statePrefixTask) : showSolution? useProjectStore(s => s.project.config?.statePrefixSolution) : undefined
  ...
  const runSimulation = (input: string): SimulationResult => {
    ...
    } else if (['PDA', 'FSA', 'T'].includes(graph.projectType)) {
      const result =
        graph.projectType === 'PDA'
          ? simulatePDA(graph, input ?? '')
          : graph.projectType === 'T'
            ? simulateTask(graph, input ?? '')
