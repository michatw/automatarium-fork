...
import {
  ...
  InputSpacingWrapper,
  Heading
} from './inputTransitionGroupProjectStyle'
...
import {
  ...
  SubmitButton,
  TMSubmitButton
} from '../InputDialogsProject/inputDialogsProjectStyle'
...
const InputTransitionGroupProject = () => {
  ...
  const statePrefix = showTask? useProjectStore((s) => s.project.config.statePrefixTask) : showExpression? useProjectStore((s) => s.project.config.statePrefixSolution) : undefined
  ...
  useEvent(
    'editTransitionGroup',
    ({ detail: { ctx } }) => {
      const { transitions, states } = useProjectStore.getState()?.project ?? {transitions: [], states: []} 
  ...
  const retrieveTransitions = useCallback(() => {
    const { transitions } = useProjectStore.getState()?.project ?? {transitions: [], states: []}
...
export default InputTransitionGroupProject
