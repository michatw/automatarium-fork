...
import { useExpressionStore, useProjectStore, useToolStore, useViewStore } from '/src/stores'
...
const useTransitionCreation = (mode, type): { createTransitionStart: Coordinate, createTransitionEnd: Coordinate } => {
  
  const isProject = type === PROJECT_TYPE
  const isExpression = type === EXPRESSION_TYPE

  let createTransition, moveTransition, getStates
  if (isProject)  {
    createTransition = useProjectStore(s => s.createTransition)
    moveTransition = useProjectStore(s => s.moveTransition)
  } else if (isExpression) {
    createTransition = useExpressionStore(s => s.createTransition)
    moveTransition = useExpressionStore(s => s.moveTransition)
    getStates = useExpressionStore(s => s.getStates)
  }
  ...
  useEvent('state:mousedown', e => {
    if (tool === 'transition' && e.detail.originalEvent.button === 0) {
      ...
      let states
      if (isProject) {
        states = useProjectStore.getState().project?.states ?? []
      } else if (isExpression) {
        states = getStates()
      }
  ...
  useEvent('state:mouseup', e => {
    if (createTransitionState && e.detail.originalEvent.button === 0) {
      const id = createTransition({ from: createTransitionState.id, to: e.detail.state.id, mode: mode })
