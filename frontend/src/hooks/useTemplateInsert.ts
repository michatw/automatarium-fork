...
import { InsertGroupResponseTypeProject } from '../stores/useProjectStore'
...
const useTemplateInsert = () => {
  ...
  useEvent('svg:mouseup', e => {
    ...
    if (template !== null && e.detail.didTargetSVG && e.detail.originalEvent.button === 0) {
      ...
      if (insertResponse.type === InsertGroupResponseTypeProject.SUCCESS) {
        ...
        setTool('cursor')
        setTemplate(null)
      } else if (insertResponse.type === InsertGroupResponseTypeProject.FAIL) {
